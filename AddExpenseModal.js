...
function AddExpenseModal({show,onClose}){
    ...
    return (<Modal  show={show} onClose={onClose}>
       <div className="flex flex-col gap-4">
          ...
          <input type="number" min={0.01} step={0.01} placeholder="Enter expensne Amount" value={expenseAmount} onChange={(e)=>{
       ...
       <div className="flex flex-col gap-4 mt-6 ">
          ...
          <div className='flex items-center justify-between'>
             <h3 className="text-2xl capitilize ">Select expense Category</h3>
             <button onClick={()=>{
       ...
       {expenses.map(expense=>{
        return (
        <button key={expense.id} onClick={()=>{setSelectCategory(expense.id);}}>
          <div style={ {boxShadow:expense.id==selectedCategory ?"1px 1px 4px":"none",}} className="flex item-center justify-between px-4 py-4 bg-slate-700 rounded-3xl">
